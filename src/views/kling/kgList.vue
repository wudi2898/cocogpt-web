...
import { LazyImg, Waterfall, ViewCard } from 'vue-waterfall-plugin-next'
            ...
            <template #item="{ item, url, index }">
                <div class="bg-white dark:bg-[#24272e] rounded-md   overflow-hidden cursor-pointer group/item relative">
                    <div v-if="'failed' == item.task.data.task_status">
                        ...
                        <div class="w-full h-[200px] justify-center items-center flex text-center"> {{ t('video.failed')
                            }}<br>ID: {{ item.task.data.task_id }}</div>

                    </div>
                    <template
                        v-else-if="(!item.task.last_feed || ((new Date().getTime()) - item.task.last_feed) > 20 * 1000) && !item.src">
                    ...
                    <template v-else>
                        <div v-if="item.task.cat != 'image' && item.src" style="padding-bottom: 0%;">
                            <video v-if="item.src" :src="item.src" loop playsinline disableremoteplayback
                                disablepictureinpicture :controls="st.pIndex == index"
                        ...
                        <section class="absolute top-0 left-0 right-0 bottom-0"
                    ...
                    <section v-if="item.task.prompt" class="absolute w-full bottom-0   backdrop-blur-sm text-white/70  "
